...
const VISA_OPTIONS: {
  ...
  { value: 'GC',       label: 'Green Card', desc: 'Permanent resident',            bg: 'bg-gold/15',text: 'text-gold-deep',border: 'border-gold/40',ring: 'ring-gold'},
  { value: 'CITIZEN',  label: 'US Citizen', desc: 'No sponsorship needed',         bg: 'bg-paper',   text: 'text-ink-soft',   border: 'border-line',   ring: 'ring-gray-400'   },
  { value: 'OTHER',    label: 'Other',      desc: 'Other visa status',             bg: 'bg-paper',   text: 'text-ink-soft',   border: 'border-line',   ring: 'ring-gray-300'   },
]
...
const STATUS_META: Record<AppStatus, { label: string; text: string; bg: string; bar: string; dot: string }> = {
  ...
  offer:        { label: 'Offer',         text: 'text-gold-deep', bg: 'bg-gold/15', bar: 'bg-gold', dot: 'bg-gold' },
  rejected:     { label: 'Rejected',      text: 'text-red-600',     bg: 'bg-red-50',     bar: 'bg-red-400',     dot: 'bg-red-400'     },
  withdrawn:    { label: 'Withdrawn',     text: 'text-muted',    bg: 'bg-line',   bar: 'bg-line',    dot: 'bg-line'    },
}
...
function Avatar({ email }: { email: string }) {
  ...
  return (
    <div className="relative">
      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-brand to-[#0f7a34] flex items-center justify-center text-white text-2xl font-black shadow-lg border-4 border-white/20">
        {initials}
      </div>
      <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-gold border-2 border-white rounded-full" />
    </div>
  )
}
...
function MetricCard({ icon, value, label, sub, accent }: {
  ...
  return (
    <div className="bg-paper-raised rounded-2xl border border-line p-4 hover:shadow-sm transition-shadow">
      ...
      <p className="text-sm font-semibold text-ink-soft mt-1">{label}</p>
...
function UnauthenticatedView() {
  return (
    <div className="min-h-screen bg-paper">
      {/* Hero */}
      <div className="bg-gradient-to-br from-ink via-ink to-brand px-6 py-16 text-center">
        ...
        <h1 className="font-display text-3xl font-bold text-white mb-3">Your Profile</h1>
      ...
      <div className="max-w-md mx-auto px-6 -mt-6">
        <div className="bg-paper-raised rounded-2xl shadow-sm border border-line divide-y divide-gray-50 overflow-hidden">
          {[
            { icon: <BarChart3 className="w-4 h-4 text-brand" />,   title: 'Application Dashboard',   desc: 'Pipeline view of every application stage' },
            ...
            <div key={f.title} className="flex items-center gap-4 px-5 py-4">
              <div className="w-8 h-8 bg-paper rounded-xl flex items-center justify-center shrink-0">{f.icon}</div>
              <div>
...
function LoadingSkeleton() {
  return (
    <div className="min-h-screen bg-paper">
      <div className="bg-gradient-to-br from-ink to-brand h-44" />
      <div className="max-w-2xl mx-auto px-6 -mt-6 space-y-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="bg-paper-raised rounded-2xl border border-line p-5 space-y-3">
...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      {/* ── Hero banner ── */}
      <div className="bg-gradient-to-br from-ink via-ink to-brand-deep pb-0">
        <div className="max-w-2xl mx-auto px-6 pt-8 pb-6">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-5">
              <Avatar email={user.email ?? 'U'} />
              <div>
                <h1 className="font-display text-xl font-bold text-white tracking-tight">{handle}</h1>
      ...
      <div className="max-w-2xl mx-auto px-6 py-6 space-y-5">
        ...
        {tab === 'overview' && (
          <>
            ...
            {applications.length > 0 ? (
              <div className="bg-paper-raised rounded-2xl border border-line p-6 shadow-sm">
                <div className="flex items-center justify-between mb-5">
                  <div>
                    ...
                    <p className="text-xs text-muted mt-0.5">{applications.length} total applications</p>
                  </div>
                  ...
                  {Object.entries(STATUS_META).map(([s, m]) => {
                    ...
                    return (
                      <div key={s} className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-ink-soft flex-1">{m.label}</span>
                        <div className="w-28 h-1.5 bg-line rounded-full overflow-hidden">
                          <div className={cn('h-full rounded-full', m.bar)} style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-xs font-bold text-muted w-5 text-right">{n}</span>
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="bg-paper-raised rounded-2xl border border-dashed border-line p-10 text-center shadow-sm">
                ...
                <p className="font-bold text-ink-soft mb-1">No applications yet</p>
                <p className="text-sm text-muted mb-5">Browse jobs and log your first application in one click</p>
                <Link href="/jobs"
            ...
            {applications.length > 0 && (
              <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-line">
                  ...
                  <Link href="/applications" className="text-xs font-semibold text-brand hover:underline flex items-center gap-1">
                ...
                <div className="divide-y divide-gray-50">
                  {applications.slice(0, 6).map(app => {
                    ...
                    return (
                      <div key={app.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-paper/50 transition-colors">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 border border-line flex items-center justify-center shrink-0">
                          <Building2 className="w-4 h-4 text-muted" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-ink truncate">{app.company_name}</p>
            ...
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-line">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-ink">My Resume</p>
                  <p className="text-xs text-muted">Used for job matching in the &ldquo;For You&rdquo; tab</p>
                </div>
                {resume && (
              ...
              {resume && !showReplaceResume ? (
                <div className="px-5 py-4 space-y-3">
                  ...
                  <p className="text-xs text-muted bg-paper rounded-xl p-3 leading-relaxed line-clamp-3 font-mono">
                ...
                <div className="px-5 py-4 space-y-3">
                  {!resume && (
                    <p className="text-sm text-muted">No resume saved yet. Paste text or upload a .txt file to get job matches.</p>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 border border-line hover:border-violet-300 hover:text-violet-600 text-ink-soft text-xs font-semibold rounded-lg transition-colors">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full px-3 py-2 text-xs border border-line rounded-xl focus:outline-none focus:ring-2 focus:ring-brand resize-none font-mono"
                  />
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleSaveResumePaste}
                      disabled={resumeSaving || !resumePaste.trim()}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-brand hover:bg-brand-deep text-white text-xs font-bold rounded-lg disabled:opacity-40 transition-colors">
                      {resumeSaving ? 'Saving…' : 'Save Resume'}
                    </button>
                    {showReplaceResume && (
                      <button
                        onClick={() => { setShowReplaceResume(false); setResumePaste('') }}
                        className="text-xs text-muted hover:text-ink-soft px-2 py-1.5">
                        Cancel
                      </button>
                    )}
                    {resumeSaveOk && (
            ...
            {/* Nav shortcuts */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
              <p className="px-5 pt-4 pb-2 text-[11px] font-bold text-muted uppercase tracking-widest">Quick Access</p>
              {[
                { href: '/jobs',         icon: <Sparkles className="w-4 h-4 text-brand" />,   accent: 'bg-brand/10',  label: 'Browse Jobs',        desc: 'Jobs with H-1B sponsorship data' },
                ...
                <Link key={item.href} href={item.href}
                  className="flex items-center gap-3 px-5 py-3.5 hover:bg-paper transition-colors border-t border-line first-of-type:border-0">
        ...
        {tab === 'visa' && (
          <>
              ...
              <div className={cn(
                ...
                <div className="flex items-start gap-3">
                  <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center shrink-0',
                    tip.urgent ? 'bg-amber-100' : 'bg-brand/15')}>
                    {tip.urgent
                      ? <Clock className="w-4 h-4 text-amber-600" />
                      : <CheckCircle className="w-4 h-4 text-brand" />}
                  </div>
                  <div>
                    <p className={cn('font-bold text-sm', tip.urgent ? 'text-amber-800' : 'text-brand-deep')}>
                      {tip.headline}
                    </p>
                    <p className={cn('text-sm mt-1 leading-relaxed', tip.urgent ? 'text-amber-700' : 'text-brand-deep')}>
                      {tip.body}
                    </p>
                    {tip.next && (
                      <div className={cn('mt-3 flex items-center gap-1.5 text-xs font-semibold',
                        tip.urgent ? 'text-amber-600' : 'text-brand')}>
            ...
            {/* Visa selector */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm p-6">
              <h2 className="font-bold text-ink mb-1">Work Authorization</h2>
              <p className="text-sm text-muted mb-5">Select your current status so we can tailor results.</p>
                  ...
                  <button key={opt.value} onClick={() => setSelected(opt.value)}
                    className={cn(
                      ...
                      selected === opt.value
                        ...
                        : 'border-line bg-paper-raised hover:border-line hover:bg-paper/70'
                    )}>
                    <span className={cn('text-sm font-bold', selected === opt.value ? '' : 'text-ink')}>
                      {opt.label}
                    </span>
                    <span className={cn('text-xs mt-1', selected === opt.value ? 'opacity-70' : 'text-muted')}>
                      {opt.desc}
                    </span>
                  </button>
                ))}
              ...
              <div className="flex items-center gap-3 mt-5">
                <button onClick={handleSave}
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-brand hover:bg-brand-deep text-white text-sm font-bold rounded-xl disabled:opacity-40 transition-colors">
                  {saving ? 'Saving…' : <><CheckCircle className="w-4 h-4" /> Save changes</>}
                </button>
                {saveOk && (
                  <span className="flex items-center gap-1.5 text-sm text-brand font-semibold">
            ...
            {/* Journey timeline */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm p-6">
              <h3 className="font-bold text-ink mb-1">International Work Journey</h3>
              <p className="text-xs text-muted mb-5">Typical path from student visa to US citizen</p>
              <div className="relative">
                {/* Vertical connector */}
                <div className="absolute left-4 top-5 bottom-5 w-0.5 bg-line" />
                    ...
                    return (
                      <div key={step.key} className="flex items-start gap-4 relative">
                        <div className={cn(
                          ...
                          realDone   ? 'bg-brand border-brand text-white' :
                          realActive ? 'bg-paper-raised border-brand text-brand ring-4 ring-brand/20' :
                                       'bg-paper-raised border-line text-muted'
                        )}>
                        ...
                        <div className={cn('flex-1 pb-1', realFuture ? 'opacity-40' : '')}>
                          <div className="flex items-center gap-2">
                            <p className={cn('text-sm font-bold',
                              realDone || realActive ? 'text-ink' : 'text-muted')}>
                              {step.label}
                            </p>
                            {realActive && (
                              <span className="px-2 py-0.5 bg-brand/15 text-brand text-[10px] font-bold rounded-full">
                                You are here
                              </span>
                            )}
                          </div>
        ...
        {tab === 'settings' && (
          <>
            {/* Account */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
              <p className="px-5 pt-4 pb-2 text-[11px] font-bold text-muted uppercase tracking-widest">Account</p>
              <div className="divide-y divide-gray-50">
                <div className="flex items-center gap-3 px-5 py-4">
                  ...
                  <span className="flex items-center gap-1 text-xs text-gold-deep font-semibold bg-gold/15 px-2.5 py-1 rounded-full border border-gold/30">
                ...
                <div className="flex items-center gap-3 px-5 py-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-ink">Visa status</p>
                    <p className="text-xs text-muted mt-0.5">{currentVisa?.label ?? 'Not set'}</p>
                  </div>
            ...
            {/* Browser extension */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
              <p className="px-5 pt-4 pb-2 text-[11px] font-bold text-muted uppercase tracking-widest">Extension</p>
              <div className="px-5 py-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-ink">JADsynq Autofill extension</p>
                    <p className="text-xs text-muted mt-0.5">Copy your account token and paste it into the extension popup to connect it.</p>
                  </div>
                </div>
                <button
                  ...
                  className={cn(
                    ...
                    tokenCopied ? 'bg-brand/10 text-brand border border-brand/30' : 'bg-paper hover:bg-line text-ink-soft border border-line'
                  )}
                >
            ...
            {/* Privacy */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm overflow-hidden">
              <p className="px-5 pt-4 pb-2 text-[11px] font-bold text-muted uppercase tracking-widest">Privacy</p>
              <div className="divide-y divide-gray-50">
                {[
                  { icon: <Shield className="w-4 h-4 text-brand" />,    accent: 'bg-brand/10',  title: 'Visa status is private',      desc: 'Only personalises your feed — never shared externally.' },
            ...
            {/* Stats snapshot */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm p-5">
              <p className="text-[11px] font-bold text-muted uppercase tracking-widest mb-4">Your Data Snapshot</p>
              <div className="grid grid-cols-3 gap-3">
                {[
                  { n: applications.length, l: 'Applications', bg: 'bg-brand/10',  t: 'text-brand' },
                  ...
                  <div key={s.l} className={cn('rounded-xl p-3 text-center', s.bg)}>
                    ...
                    <p className="text-xs text-muted mt-0.5 font-medium">{s.l}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Danger zone */}
            <div className="bg-paper-raised rounded-2xl border border-line shadow-sm p-5">
              <p className="text-[11px] font-bold text-muted uppercase tracking-widest mb-4">Session</p>
              <button onClick={signOut}
