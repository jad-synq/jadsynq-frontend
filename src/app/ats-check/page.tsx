...
function ScoreRing({ score, label, size = 'lg' }: { score: number; label: string; size?: 'sm' | 'lg' }) {
  ...
  const color = score >= 75 ? '#0E7C4A' : score >= 50 ? '#d97706' : '#dc2626'
  const sw = big ? 12 : 8
    ...
    <div className="flex flex-col items-center gap-1">
      <div className="relative flex items-center justify-center" style={{ width: dim, height: dim }}>
        ...
        <div className="flex flex-col items-center z-10">
          ...
          <span className={cn('text-muted font-semibold', big ? 'text-xs' : 'text-[10px]')}>/ 100</span>
        </div>
      </div>
      ...
      <p className={cn('text-muted text-center', big ? 'text-xs' : 'text-[10px]')}>{label}</p>
    </div>
  )
}
...
function KeywordBucket({ label, keywords, color, icon: Icon }: {
  ...
  return (
    <div>
      <p className="flex items-center gap-1.5 text-xs font-bold text-muted uppercase tracking-wide mb-1.5">
...
function ScoreBar({ label, score, max, desc }: { label: string; score: number; max: number; desc: string }) {
  const pct = score / max
  const color = pct >= 0.7 ? '#0E7C4A' : pct >= 0.4 ? '#d97706' : '#dc2626'
  return (
    <div>
      <div className="flex justify-between text-sm mb-1">
        <span className="font-medium text-ink-soft">{label}</span>
        <span className="text-muted text-xs">{score}/{max} pts</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted w-36 shrink-0 text-right">{desc}</span>
      </div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-paper">
      ...
      <div className="bg-paper-raised border-b border-line">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-2xl font-bold text-ink">ATS Score Checker</h1>
              <p className="text-sm text-muted">Keyword match · Format analysis · Cover letter generator</p>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          {/* Job Description */}
          <div className="bg-paper-raised rounded-2xl border border-line p-5">
            <label className="block text-sm font-bold text-ink-soft mb-1">
              Job Description <span className="text-red-500">*</span>
            </label>
            <p className="text-xs text-muted mb-3">Paste the full job posting including requirements</p>
            <textarea
              value={jd}
              onChange={e => setJd(e.target.value)}
              placeholder="Paste job description here…"
              className="w-full h-52 text-sm border border-line rounded-xl p-3 resize-none focus:outline-none focus:ring-2 focus:ring-brand font-mono"
            />
            <p className="text-xs text-muted mt-1 text-right">{jd.split(/\s+/).filter(Boolean).length} words</p>
          </div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleImportFromBuilder}
                  className={cn(
                    ...
                    imported ? 'bg-brand/10 text-brand-deep border-brand/30' : 'bg-violet-50 text-violet-700 border-violet-200 hover:bg-violet-100'
                  )}>
                  <Wand2 className="w-3 h-3" />{imported ? 'Imported!' : 'Import from Builder'}
                </button>
                <Link href="/resume-builder" className="flex items-center gap-1 text-xs text-brand font-semibold hover:underline">
                  <Upload className="w-3 h-3" /> Build
                </Link>
              </div>
            </div>
            <p className="text-xs text-muted mb-3">Import from Resume Builder or paste as plain text</p>
            <textarea
              value={resume}
              ...
              className="w-full h-52 text-sm border border-line rounded-xl p-3 resize-none focus:outline-none focus:ring-2 focus:ring-brand font-mono"
            />
            <p className="text-xs text-muted mt-1 text-right">{resume.split(/\s+/).filter(Boolean).length} words</p>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full py-4 bg-brand hover:bg-brand-deep disabled:bg-line disabled:text-muted text-white font-bold rounded-xl text-base transition-colors flex items-center justify-center gap-2 mb-8"
        >
          ...
          <div className="space-y-5">
            ...
            <div className="bg-paper-raised rounded-2xl border border-line p-6">
              <div className="flex flex-col sm:flex-row items-center justify-around gap-6">
                ...
                <div className="hidden sm:block h-20 w-px bg-line" />
                <ScoreRing score={readResult.score} label="Readability / Format Score" />
                <div className="hidden sm:block h-20 w-px bg-line" />
                <div className="flex flex-col items-center gap-2">
                  <div className="text-3xl font-black text-ink-soft">
                    {Math.round((atsResult.score + readResult.score) / 2)}
                  </div>
                  <div className="text-xs text-muted font-semibold">Combined Score</div>
                  <div className="flex gap-2 mt-1">
                    {[
                      ...
                      <div key={d.label} className="text-center">
                        <div className="text-sm font-bold text-ink-soft">{d.val}/{d.max}</div>
            ...
            {/* Tab nav */}
            <div className="flex gap-1 bg-paper-raised rounded-2xl border border-line p-1.5">
              {TABS.map(t => (
                <button
                  key={t.id}
                  ...
                  className={cn(
                    ...
                    activeTab === t.id
                      ...
                      : 'text-muted hover:text-ink-soft hover:bg-paper'
                  )}
                >
                  {t.label}
                </button>
            ...
            {activeTab === 'ats' && (
              <div className="space-y-5">
                {/* Breakdown */}
                <div className="bg-paper-raised rounded-2xl border border-line p-6 space-y-3">
                  <h2 className="font-bold text-ink mb-4">Score Breakdown</h2>
                ...
                <div className="bg-paper-raised rounded-2xl border border-line p-5">
                  <h3 className="font-bold text-ink mb-4 flex items-center gap-2">
                    <LayoutTemplate className="w-4 h-4 text-brand" /> Section Checklist
                  </h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {Object.entries(atsResult.sections).map(([key, found]) => (
                      <div key={key} className={cn(
                        'flex items-center gap-2 px-3 py-2 rounded-xl text-sm',
                        found ? 'bg-brand/10 text-brand-deep' : 'bg-red-50 text-red-600'
                      )}>
                ...
                <div className="bg-paper-raised rounded-2xl border border-line p-5">
                  <h3 className="font-bold text-ink mb-4 flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-brand" /> Matched Keywords
                    <span className="ml-auto text-xs font-normal text-muted">{atsResult.matched.length} found</span>
                  </h3>
                  {atsResult.matched.length === 0 ? (
                    <p className="text-sm text-muted">No matches yet — add keywords from the job description</p>
                  ) : (
                    <div className="space-y-3">
                      <KeywordBucket label="Technical Skills" keywords={atsResult.matchedBuckets.tech} color="#0E7C4A" icon={Cpu} />
                ...
                <div className="bg-paper-raised rounded-2xl border border-line p-5">
                  <h3 className="font-bold text-ink mb-4 flex items-center gap-2">
                    ...
                    <span className="ml-auto text-xs font-normal text-muted">{atsResult.missing.length} missing</span>
                  </h3>
                  {atsResult.missing.length === 0 ? (
                    <p className="text-sm text-brand-deep font-semibold">All key terms found — great match!</p>
                  ) : (
                    <div className="space-y-3">
            ...
            {activeTab === 'readability' && (
              <div className="space-y-5">
                {/* Breakdown */}
                <div className="bg-paper-raised rounded-2xl border border-line p-6 space-y-3">
                  <h2 className="font-bold text-ink mb-4">Format & Readability Breakdown</h2>
                ...
                {readResult.passes.length > 0 && (
                  <div className="bg-paper-raised rounded-2xl border border-line p-5">
                    <h3 className="font-bold text-ink mb-3 flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-brand" /> What Looks Good
                    </h3>
                    <ul className="space-y-2">
                      {readResult.passes.map((p, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-brand-deep bg-brand/10 rounded-xl px-3 py-2">
                          <CheckCircle className="w-4 h-4 shrink-0 text-brand mt-0.5" />{p}
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="space-y-5">
                <div className="bg-paper-raised rounded-2xl border border-line p-5">
                  <div className="flex items-start justify-between mb-4 gap-4">
                    <div>
                      <h3 className="font-bold text-ink">AI-Assisted Cover Letter</h3>
                    ...
                    <div className="flex gap-2 shrink-0">
                      {coverGenerated && (
                        <button onClick={handleCopyCover}
                          className={cn(
                            ...
                            copied ? 'bg-brand/10 text-brand-deep border-brand/30' : 'bg-paper text-ink-soft border-line hover:bg-line'
                          )}>
                      ...
                      <button onClick={handleGenerateCoverLetter}
                        className="flex items-center gap-1.5 px-3 py-2 bg-brand hover:bg-brand-deep text-white text-xs font-bold rounded-xl transition-colors">
                  ...
                  {!coverGenerated ? (
                    <div className="border-2 border-dashed border-line rounded-xl p-8 text-center">
                      <Wand2 className="w-8 h-8 mx-auto mb-3 text-muted" />
                      <p className="text-sm font-semibold text-muted">Click Generate to create a cover letter</p>
                    ...
                    <textarea
                      ...
                      className="w-full h-96 text-sm border border-line rounded-xl p-4 resize-none focus:outline-none focus:ring-2 focus:ring-brand leading-relaxed"
                    />
                  )}
                </div>
            ...
            <div className="flex gap-3">
              <Link href="/resume-builder"
                className="flex items-center gap-2 px-5 py-3 bg-brand hover:bg-brand-deep text-white font-bold rounded-xl text-sm transition-colors">
              ...
              <button
                ...
                className="px-5 py-3 bg-paper-raised border border-line hover:border-line text-ink-soft font-semibold rounded-xl text-sm transition-colors">
                Start Over
              </button>
            </div>
          </div>
        ...
        {!analyzed && (
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={item.step} className="bg-paper-raised rounded-2xl border border-line p-5">
                <div className="w-8 h-8 bg-brand text-white rounded-xl flex items-center justify-center font-black text-sm mb-3">{item.step}</div>
